...
SELECT SUM(PRICE)
FROM PRODUCTS
WHERE BRAND = 'APC'; -- select only APC 俾條件
...
-- NEW TABLE
CREATE TABLE ORDERS1 (  -- TABLE CL0NUMS
ORDER_ID INTEGER, 
SKU VARCHAR (10),
PRICE DECIMAL (11,2),
QTY INTEGER
...
INSERT INTO ORDERS1 (ORDER_ID, SKU, PRICE, QTY)
VALUE (1, '1234', 500, 1);
...
SELECT SKU AS ORDERED_ITEMS  -- SHOW ORDERED ITEMS FROM PRODUCTLIST
FROM PRODUCTS
where SKU in(
    select SKU
    from  ORDERS1
